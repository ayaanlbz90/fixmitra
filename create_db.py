...
def build_database():
    ...
    # NOTE: the original version of this script passed the hashed password as
    # a bind parameter to a query that had no placeholder for it, so the
    # plaintext password ('2007') was the value actually stored. Fixed below
    # so the hash is what gets written to the users table.
    hashed_password = generate_password_hash('password123')
    cursor.execute(
        '''INSERT INTO users (name, email, phone, password, role)
           VALUES (?, ?, ?, ?, ?)''',
        ('Ayaan Shaikh', 'ayaan@fixmitra.com', '+919558533117', hashed_password, 'admin')
    )

    demo_techs = [
        ("Khadim Service", 4.9, 1420, 1.8, 90, "Mobile & Laptop Expert", 1),
        ("Precision Micro-Fix Services", 4.8, 980, 2.4, 90, "PC & Laptop Specialist", 1),
        ("Milan Mobile", 4.7, 650, 3.1, 60, "Chip Level Repair", 1),
        ("Fonebook", 4.6, 1120, 4.0, 30, "Screen & Battery Fast Service", 1)
    ]
    cursor.executemany(
    ...
    print("Admin login -> email: ayaan@fixmitra.com | password: password123")
